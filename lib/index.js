...
async function arab(opts = {}) {
      ...
      try {
        file.contents = Buffer.from(
          babelTransform({
            ...
            loggerPrefix:
              opts.name && opts.monorepo && watch ? opts.name : false,
          })
        )
        // .(jsx|tsx) -> .js
  ...
  const ps = srcFn(
     ...
     */
    .pipe(gulpif(f => TS_REG.test(f.path) || JS_REG.test(f.path), gulpBabel()))
    /**
     * Transpile `*.less`.
     */
    .pipe(gulpif(f => LESS_REG.test(f.path), less()))
    /**
     * Apply postcss.
     */
    .pipe(
